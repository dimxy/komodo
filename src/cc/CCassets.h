...
bool ValidateAssetOpret(CTransaction tx, int32_t v, uint256 refassetid, int64_t &price, std::vector<uint8_t> &origpubkey);
...
// bool AssetExactAmounts(int32_t maxDepth, struct CCcontract_info *cp,int64_t &inputs,int32_t starti,int64_t &outputs,Eval* eval,const CTransaction &tx,uint256 assetid); -- moved to CCinclude (dimxy)
