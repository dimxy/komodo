...
std::string CreateBuyOffer(int64_t txfee, int64_t bidamount, uint256 assetid, int64_t pricetotal)
{
    ...
    if (bidamount < 0 || pricetotal < 0)
    {
        ...
        CCerror = "negative bidamount or price";
        return("");
    }
    ...
    {
        fprintf(stderr,"assetid isn't assetcreation txid\n");
        return("");
    }
...
std::string CreateSell(int64_t txfee,int64_t askamount,uint256 assetid,int64_t pricetotal)
{
    ...
    if (askamount < 0 || pricetotal < 0)    {
        fprintf(stderr,"negative price %lld, askamount %lld\n",(long long)pricetotal,(long long)askamount);
        CCerror = "negative price or askamount";
        return("");
    }
    ...
    if (AddNormalinputs(mtx, mypk, 2*txfee, 3) > 0)
    {
        ...
        if ((inputs = AddTokenCCInputs(cpTokens, mtx, mypk, assetid, askamount, 60, vopretNonfungible)) > 0)
        {
			if (inputs < askamount) {
				...
				std::cerr << __func__ << "insufficient tokens for ask" << std::endl;
...
std::string CancelBuyOffer(int64_t txfee,uint256 assetid,uint256 bidtxid)
{
    ...
    if (AddNormalinputsRemote(mtx, mypk, txfee, 3) > 0)
    {
        ...
        if (myGetTransaction(bidtxid, vintx, hashBlock) != 0)
        {
            ...
            CCerror = "could not load bid txid";
    }
    else
...
std::string CancelSell(int64_t txfee,uint256 assetid,uint256 asktxid)
{
    ...
    if (AddNormalinputsRemote(mtx, mypk, txfee, 3) > 0)
    {
        ...
        if (myGetTransaction(asktxid, vintx, hashBlock) != 0)
        {
            ...
            CCerror = "could lnot get ask txid";
    }
    else
        CCerror = "could not get normal coins for txfee";
    return("");
...
std::string FillBuyOffer(int64_t txfee,uint256 assetid,uint256 bidtxid,int64_t fill_units)
{
	...
	int32_t bidvout=0; 
	uint64_t mask; 
	int64_t orig_units, bid_amount, paid_amount, remaining_units_required, inputs, CCchange=0; 
	struct CCcontract_info *cpTokens, tokensC;
	struct CCcontract_info *cpAssets, assetsC;

    if (fill_units < 0)
    {
        fprintf(stderr,"negative fill_units %lld\n", (long long)fill_units);
    ...
    if (AddNormalinputs(mtx, mypk, 2*txfee, 3) > 0)
    {
        ...
        if (myGetTransaction(bidtxid, vintx, hashBlock) != 0 && vintx.vout.size() > bidvout)
        {
            ...
            SetAssetOrigpubkey(origpubkey, orig_units, vintx);  // get orig pk, orig units

            mtx.vin.push_back(CTxIn(bidtxid, bidvout, CScript()));					// Coins on Assets unspendable

            std::vector<uint8_t> vopretNonfungible;
            if ((inputs = AddTokenCCInputs(cpTokens, mtx, mypk, assetid, fill_units, 60, vopretNonfungible)) > 0)
            {
                if (inputs < fill_units) {
                    std::cerr << __func__ << ": insufficient tokens to fill buy offer" << " inputs=" << inputs << " fill_units=" << fill_units << std::endl;
                    CCerror = strprintf("insufficient tokens to fill buy offer");
                    return ("");
                }

                SetBidFillamounts(paid_amount, remaining_units_required, bid_amount, fill_units, orig_units);
                ...
                mtx.vout.push_back(MakeTokensCC1vout(additionalTokensEvalcode2 == 0 ? EVAL_TOKENS : additionalTokensEvalcode2, fill_units, pubkey2pk(origpubkey)));	// vout2 single-eval tokens sent to the originator
                ...
                fprintf(stderr, "%s remaining required units %llu -> origpubkey\n", __func__, (long long)remaining_units_required);
                ...
                return(FinalizeCCTx(mask, cpTokens, mtx, mypk, txfee,
                    EncodeTokenOpRet(assetid, voutTokenPubkeys,
                        std::make_pair(OPRETID_ASSETSDATA, EncodeAssetOpRet('B', zeroid, remaining_units_required, origpubkey)))));
            }
            else {
...
std::string FillSell(int64_t txfee, uint256 assetid, uint256 assetid2, uint256 asktxid, int64_t fillunits)
{
	...
	uint64_t mask = 0; 
	int32_t askvout = 0; 
	int64_t orig_nValue, orig_assetoshis, paid_nValue, remaining_nValue, inputs, CCchange = 0LL; 
	//struct CCcontract_info *cpTokens, tokensC;
	struct CCcontract_info *cpAssets, assetsC;
    ...
    if (myGetTransaction(asktxid, vintx, hashBlock) != 0 && vintx.vout.size() > askvout)
    {
        ...
        SetAssetOrigpubkey(origpubkey, orig_nValue, vintx); // get orig pk, orig value
        ...
        if (inputs > 0)
        {
			if (inputs < paid_nValue) {
				std::cerr << __func__ << " insufficient coins to fill sell" << " inputs=" << inputs << std::endl;
			...
			if (assetid2 != zeroid)
                SetSwapFillamounts(fillunits, remaining_nValue, orig_assetoshis, paid_nValue, orig_nValue);  //not implemented correctly yet
            else 
				SetAskFillamounts(fillunits, remaining_nValue, orig_assetoshis, paid_nValue, orig_nValue);
            ...
            mtx.vout.push_back(MakeTokensCC1vout(EVAL_ASSETS, additionalTokensEvalcode2, orig_assetoshis - fillunits, GetUnspendable(cpAssets, NULL))); 
            //vout.1 purchased tokens to self token single-eval or dual-eval token+nonfungible cc addr:
            mtx.vout.push_back(MakeTokensCC1vout(additionalTokensEvalcode2 == 0 ? EVAL_TOKENS : additionalTokensEvalcode2, fillunits, mypk));					
