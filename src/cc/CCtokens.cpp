...
std::string CreateTokenExt(int64_t txfee, int64_t tokensupply, std::string name, std::string description, vscript_t nonfungibleData, uint8_t additionalMarkerEvalCode, bool addTxInMemory)
{
    ...
    CAmount totalInputs;
	//if ((totalInputs = AddNormalinputs(mtx, mypk, tokensupply + txfeeCount * txfee, 64, true)) > 0)
    if ((totalInputs = AddNormalinputs2(mtx, tokensupply + txfeeCount * txfee, 64, true)) > 0)  // use which can look in mempool
	{
