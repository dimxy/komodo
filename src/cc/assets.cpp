...
bool AssetsValidate(struct CCcontract_info *cp,Eval* eval,const CTransaction &tx, uint32_t nIn)
{
    ...
    else if ( funcid != 'c' )
    {
        ...
        else if ( AssetExactAmounts(true, cp,inputs,starti,outputs,eval,tx,assetid) == false )  // first-param = 'true' means to do compare inputs=outputs also for first-level ancestor tx's
    ...
    switch ( funcid )
    {
        ...
        case 'E': // fillexchange	
            ...
            //vout.n-1: opreturn [EVAL_ASSETS] ['E'] [assetid vin0+1] [assetid vin2] [remaining asset2 required] [origpubkey]
            if ( AssetExactAmounts(true, cp,inputs,1,outputs,eval,tx,assetid2) == false )    
