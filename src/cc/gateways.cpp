...
CScript EncodeGatewaysBindOpRet(uint8_t funcid,std::string coin,uint256 tokenid,int64_t totalsupply,uint256 oracletxid,uint8_t M,uint8_t N,std::vector<CPubKey> pubkeys,uint8_t taddr,uint8_t prefix,uint8_t prefix2)
{
	...
	tokenid = revuint256(tokenid);  // dimxy to match the Assets behaviour
...
CScript EncodeGatewaysClaimOpRet(uint8_t funcid,uint256 assetid,std::string refcoin,uint256 bindtxid,uint256 deposittxid,CPubKey destpub,int64_t amount)
{
	...
	assetid = revuint256(assetid);  // dimxy to match the Assets behaviour
...
int64_t IsGatewaysvout(bool compareTotals, struct CCcontract_info *cpGateways, Eval* eval, uint256 assetid, const CTransaction& tx, int32_t v)
{
    ...
    if( tx.vout[v].scriptPubKey.IsPayToCryptoCondition() )
    {
		...
		// NOTE: I turned off this check too because it is too strict (dimxy)
		// we cannot be sure that ancestor txes are only Assets'
...
int64_t AddGatewaysInputs(struct CCcontract_info *cp,CMutableTransaction &mtx,CPubKey pk,uint256 refassetid,int64_t total,int32_t maxinputs)
{
    ...
    for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++)
    {
        ...
        if ( GetTransaction(txid,vintx,hashBlock,false) != 0 )
        {
			...
			bool isEof = false;
			bool unmarshalResult = E_UNMARSHAL(vopret, ss >> evalcode; ss >> funcid; ss >> assetid; isEof = ss.eof());  // let's detect eof state if there is a remainder after standard assets' fields
			// if( E_UNMARSHAL(vopret,ss >> evalcode; ss >> funcid; ss >> assetid) != 0 )
			if (unmarshalResult || !isEof)
            {
                assetid = revuint256(assetid);  // TODO: it is better to use standard DecodeAssetOpret function which does this itself (dimxy)
