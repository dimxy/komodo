...
public:
	...
	static uint8_t getMyOpretEval() { return EVAL_HEIR; }
...
public:
	...
	static bool isMyFuncId(uint8_t funcid) { return IS_CHARINSTR(funcid, "GBt"); }   // NOTE: 't' is from Assets contract
	static uint8_t getMyOpretEval() { return EVAL_ASSETS; }
	static int64_t addOwnerInputs(struct CCcontract_info* cp, uint256 assetid, CMutableTransaction& mtx, CPubKey ownerPubkey, int64_t total, int32_t maxinputs) {
		return AddHeirTokenInputs(cp, mtx, ownerPubkey, assetid, total, maxinputs);
	}
	...
	static CScript makeCreateOpRet(uint256 assetid, CPubKey ownerPubkey, CPubKey heirPubkey, int64_t inactivityTimeSec, std::string heirName) {
		return EncodeHeirAssetsCreateOpRet((uint8_t)EVAL_ASSETS, (uint8_t)'G', assetid, ownerPubkey, heirPubkey, inactivityTimeSec, heirName);
	}
	static CScript makeAddOpRet(uint256 assetid, CPubKey ownerPubkey, CPubKey heirPubkey, int64_t inactivityTimeSec, uint256 fundingtxid) {
		return EncodeHeirAssetsOpRet((uint8_t)EVAL_ASSETS, (uint8_t)'B', assetid, ownerPubkey, heirPubkey, inactivityTimeSec, fundingtxid);
	}
	static CScript makeClaimOpRet(uint256 assetid, CPubKey ownerPubkey, CPubKey heirPubkey, int64_t inactivityTimeSec, uint256 fundingtxid) {
		return EncodeHeirConvertedAssetsOpRet((uint8_t)EVAL_ASSETS, (uint8_t)'t', assetid, ownerPubkey, heirPubkey, inactivityTimeSec, fundingtxid);
	}
	...
	static void UnmarshalOpret(std::vector<uint8_t> vopret, uint8_t &e, uint8_t &funcId, uint256 &assetid, CPubKey& ownerPubkey, CPubKey& heirPubkey, int64_t& inactivityTime, std::string& heirName, uint256& txidInOpret) {
		E_UNMARSHAL(vopret, { ss >> e; ss >> funcId; ss >> assetid; ss >> ownerPubkey; ss >> heirPubkey; ss >> inactivityTime; if (IS_CHARINSTR(funcId, "G")) { ss >> heirName; } if (IS_CHARINSTR(funcId, "Bt")) { ss >> txidInOpret; } });
	}
	static bool isSpendingTx(uint8_t funcid) { return (funcid == 't'); }

	static CTxOut makeUserVout(int64_t amount, CPubKey myPubkey) {
		return MakeCC1vout(EVAL_HEIR, amount, myPubkey);
	}
	static CTxOut makeClaimerVout(int64_t amount, CPubKey myPubkey) {
...
public:
	...
	virtual bool validateVout(CTxOut vout, std::string& message) const
	{
		...
		if (/*funcId != 'F' && funcId !='G' && txidInOpRet != zeroid ||*/					// if it is 'F' or 'G' there should be no txidInOpRet 
			...
			!IS_CHARINSTR(lastFuncId, "FG") && txidInOpRet != lastTxidInOpret) {   // possible only if last tx opRet is corrupted 
