    ...
    if (tokentx.vout.size() > 0 /* && DecodeTokenOpRetV1(tokentx.vout.back().scriptPubKey, tokenid, pks, oprets) != 0*/)
    {
        ...
        for (auto const &vin : tokentx.vin)
        {
            if (cpTokens->ismyvin(vin.scriptSig))
            {
                ...
                //else
                //{
                //    CCerrorMT::set("can't load or decode prev token tx");
...
static UniValue CreateGameObjectNFT(const CPubKey &remotepk, const struct KogsBaseObject *baseobj)
{
    ...
    if (vnftdata.empty())
    {
        CCerrorMT::set(std::string("can't marshal object with id=") + std::string(1, (char)baseobj->objectType));
...
static UniValue CreateEnclosureTx(const CPubKey &remotepk, const KogsBaseObject *baseobj, bool isSpendable, uint32_t batonType)
{
    ...
    if (AddNormalinputsRemote(mtx, mypk, normals, 0x10000, NULL, true) > 0)   // use remote version to add inputs from mypk
    {
        ...
        if (batonType)
        {
            if (batonType == BATON_GLOBAL)
                ...
                CCerrorMT::set("invalid baton type");
                return NullUniValue;
            }
        }
        ...
        if (ResultHasTx(sigData))
            ...
            CCerrorMT::set("can't finalize or sign tx");
    }
    else
        CCerrorMT::set("can't find normals for 2 txfee");
    return NullUniValue;
}
...
// nvout param:
// KOGS_USE_LAST_VOUT_OPRETURN - check only last vout opreturn (this is used for kogs operations like add/remove kogs to container, deposit containers to a game etc) 
// KOGS_TRY_OPRETURN_THEN_SEARCH_TOKEN_VOUT - first check last vout opreturn then search for opdrop vout 
// other value - check only this vout 
...
static void AddGameFinishedInOuts(const CPubKey &remotepk, CMutableTransaction &mtx, struct CCcontract_info *cpTokens, uint256 prevtxid, int32_t prevn, const std::vector<std::pair<uint256, int32_t>> &randomUtxos, const KogsBaton *pbaton, CScript &opret, bool forceFinish)
{
    ...
    // add probe to spend marker
    /*CC* probeCondKogsPk = MakeCCcond1(EVAL_KOGS, kogsPk);  
    CCAddVintxCond(cpTokens, probeCondKogsPk, kogspriv);  
    cc_free(probeCondKogsPk);*/
...
static bool AddTransferBackTokensVouts(const CPubKey &mypk, CMutableTransaction &mtx, struct CCcontract_info *cpTokens, uint256 gameid, const std::vector<std::shared_ptr<KogsContainer>> &containers, const std::vector<std::shared_ptr<KogsMatchObject>> &slammers, std::vector<CTransaction> &transferContainerTxns)
{
    ...
    for (const auto &tp : tokenspks)
    {
        ...
        if (AddTokenCCInputs(cpTokens, mtxDummy, tokensrcaddr, tp.first, 1, 5, true) > 0)  // check if container not transferred yet
        {
            ...
            {
                LOGSTREAMFN("kogs", CCLOG_ERROR, stream << "could not create transfer container back tx=" << HexStr(E_MARSHAL(ss << transfertx)) << " for containerid=" << c->creationtxid.GetHex() << " CCerrorMT=" << CCerrorMT << std::endl);
                isError = true;
...
static UniValue CreateBatonTx(const CPubKey &remotepk, uint256 prevtxid, int32_t prevn, const std::vector<std::pair<uint256, int32_t>> &randomUtxos, const KogsBaton *pbaton, const CPubKey &destpk, bool isFirst)
{
    ...
    if (AddNormalinputsRemote(mtx, mypk, txfee, 0x10000, NULL, true) > 0)
    {
        mtx.vin.push_back(CTxIn(prevtxid, prevn));  // spend the prev game or slamparam baton
...
/*static UniValue CreateSlamParamTx(const CPubKey &remotepk, uint256 prevtxid, int32_t prevn, const KogsSlamParams &slamparam)
{
    ...
    if (AddNormalinputsRemote(mtx, mypk, 3*txfee, 0x10000, NULL, true) > 0)  //use remote version to spend from mypk
    {
        ...
        if (!ResultHasTx(sigData)) {
            CCerrorMT::set("could not finalize or sign slam param transaction");
            return NullUniValue;
        }
        else
        {
    ...
    {
        CCerrorMT::set("could not find normal inputs for txfee");
...
static UniValue CreateAdvertisingTx(const CPubKey &remotepk, const KogsAdvertising &ad)
{
    ...
    if (AddNormalinputsRemote(mtx, mypk, txfee, 0x10000, NULL, true) > 0)   // add always from mypk because it will be checked who signed this advertising tx
    {
        ...
        UniValue sigData = FinalizeCCTxExt(isRemote, 0, cp, mtx, mypk, txfee, opret, false);
        if (!ResultHasTx(sigData)) {
            CCerrorMT::set("could not finalize or sign advertising transaction");
            return NullUniValue;
        }
        return sigData;
    }
...
std::vector<UniValue> KogsCreateMatchObjectNFTs(const CPubKey &remotepk, std::vector<KogsMatchObject> & matchobjects)
{
    ...
    if (!CheckSysPubKey())  {
        CCerrorMT::set("not sys pubkey used or sys pubkey not set");
        return NullResults;
    }
...
UniValue KogsStartGame(const CPubKey &remotepk, const KogsGame &newgame)
{
    ...
    {
        CCerrorMT::set("can't load game config");
        return NullUniValue;
    }

    if (newgame.playerids.size() < 2)   {
        CCerrorMT::set("number of players too low");
        return NullUniValue;
    }
    ...
    for (auto const &playerid : newgame.playerids) 
    {
        ...
        std::shared_ptr<KogsBaseObject> spPlayer( LoadGameObject(playerid) );
        if (spPlayer == nullptr || spPlayer->objectType != KOGSID_PLAYER)   {
            CCerrorMT::set("invalid playerid: " + playerid.GetHex());
            return NullUniValue;
        }

        if (!FindAdvertisings(playerid, adtxid, advout, dummy)) {
            CCerrorMT::set("playerid did not advertise itself: " + playerid.GetHex());
            return NullUniValue;
        }
...
UniValue KogsCreateFirstBaton(const CPubKey &remotepk, uint256 gameid)
{
    ...
    if (spPrevObj.get() != nullptr)
    {
        ...
        {    
            ...
            CCerrorMT::set("can't create baton object (check if all players deposited containers and slammers and added randoms)");
            return NullUniValue;
        }
    }
    else
    {
        CCerrorMT::set("can't load game");
        return NullUniValue;
    }
}
...
std::vector<UniValue> KogsCreateContainerV2(const CPubKey &remotepk, KogsContainer newcontainer, const std::set<uint256> &tokenids)
{
    ...
    if (spplayer == nullptr || spplayer->objectType != KOGSID_PLAYER)
    {
        CCerrorMT::set("could not load this playerid");
        return NullResults;
    }
    ...
    {
        CCerrorMT::set("not your playerid");
        return NullResults;
    }
    ...
    {
        CCerrorMT::set("can't unmarshal container tx");
        return NullResults;
    } */
...
static UniValue SpendEnclosure(const CPubKey &remotepk, int64_t txfee, KogsEnclosure enc, CPubKey destpk)
{
    ...
    if (AddNormalinputs(mtx, mypk, txfee, 0x10000, isRemote) > 0)
    {
        if (enc.latesttxid.IsNull()) {
            CCerrorMT::set(strprintf("incorrect latesttx in container");
            return NullUniValue;
        }
        ...
        if (!ResultHasTx(sigData))
        {
            CCerrorMT::set(strprintf("could not finalize transfer container tx");
            return nullUnivalue;
        }
    ...
    {
        CCerrorMT::set("insufficient normal inputs for tx fee");
    }
    return NullUniValue;
}
*/
...
UniValue KogsDepositTokensToGame(const CPubKey &remotepk, CAmount txfee_, uint256 gameid, uint256 containerid, uint256 slammerid)
{
    ...
    if (spgamebaseobj == nullptr || spgamebaseobj->objectType != KOGSID_GAME) {
        CCerrorMT::set("can't load game data");
        return NullUniValue;
    }
    ...
    if (spgameconfigbaseobj == nullptr || spgameconfigbaseobj->objectType != KOGSID_GAMECONFIG) {
        CCerrorMT::set("can't load game config data");
        return NullUniValue;
    }
    KogsGameConfig *pgameconfig = (KogsGameConfig *)spgameconfigbaseobj.get();

    std::shared_ptr<KogsBaseObject>spcontbaseobj(LoadGameObject(containerid, KOGS_TRY_OPRETURN_THEN_SEARCH_TOKEN_VOUT));
    if (spcontbaseobj == nullptr || spcontbaseobj->objectType != KOGSID_CONTAINER) {
        CCerrorMT::set("can't load container");
        return NullUniValue;
    }
    ...
    if (pcontainer->tokenids.size() != pgameconfig->numKogsInContainer)     {
        CCerrorMT::set("kogs number in container does not match game config");
        return NullUniValue;
    }

    std::shared_ptr<KogsBaseObject>spslammerbaseobj(LoadGameObject(slammerid, KOGS_TRY_OPRETURN_THEN_SEARCH_TOKEN_VOUT));
    if (spslammerbaseobj == nullptr || spslammerbaseobj->objectType != KOGSID_SLAMMER) {
        CCerrorMT::set("can't load slammer");
        return NullUniValue;
    }
    ...
    if (ResultIsError(addContResult)) {
        CCerrorMT::set(ResultGetError(addContResult));
        return NullUniValue;
    }
    ...
    if (ResultIsError(addSlammerResult)) {
        CCerrorMT::set(ResultGetError(addSlammerResult));
        return NullUniValue;
    }
...
UniValue KogsClaimDepositedToken(const CPubKey &remotepk, CAmount txfee_, uint256 gameid, uint256 tokenid)
{
    ...
    std::shared_ptr<KogsBaseObject>spnftobj(LoadGameObject(tokenid, KOGS_TRY_OPRETURN_THEN_SEARCH_TOKEN_VOUT));
    if (spnftobj == nullptr || spnftobj->objectType != KOGSID_CONTAINER && spnftobj->objectType != KOGSID_SLAMMER) {
        CCerrorMT::set("can't load container or slammer");
        return NullUniValue;
    }
    ...
    if (GetNFTUnspentTx(tokenid, lasttx))
    {
        ...
        if (vpks0.size() != 1)    {
            CCerrorMT::set("could not get cc vin pubkey");
            return NullUniValue;
        }
        if (mypk != vpks0[0]) {
            CCerrorMT::set("not your pubkey deposited token");
            return NullUniValue;
        }
        ...
        if (ResultIsError(addtxResult)) {
            CCerrorMT::set(ResultGetError(addtxResult));
            return NullUniValue;
        }
        ...
        CCerrorMT::set("cant get last tx for container");

    //}
    //else
    //    CCerrorMT::set("not my container");
...
static int CheckIsMyContainer(const CPubKey &remotepk, uint256 gameid, uint256 containerid)
{
    ...
    if (!gameid.IsNull())
    {
        ...
        if (IsContainerDeposited(*pgame, *pcontainer)) 
        {
            if (mypk != pgame->encOrigPk) {  // TODO: why is only game owner allowed to modify container?
                CCerrorMT::set("can't add or remove kogs: container is deposited and you are not the game creator");
                return 0;
            }
            else
                return 1;
    ...
    if (!GetNFTUnspentTx(containerid, lasttx)) {
        CCerrorMT::set("container is already burned or not yours");
        return -1;
    }
    if (lasttx.vout.size() < 1) {
        CCerrorMT::set("incorrect nft last tx");
        return -1;
    }
    for(const auto &v : lasttx.vout)    {
    ...
    CCerrorMT::set("this is not your container to add or remove kogs");
    return 0;
}
...
std::vector<UniValue> KogsAddKogsToContainerV2(const CPubKey &remotepk, int64_t txfee, uint256 containerid, const std::set<uint256> &tokenids)
{
    ...
    {
        CCerrorMT::set(ResultGetError(sigData));
    }
...
UniValue KogsCreateSlamData(const CPubKey &remotepk, KogsSlamData &newSlamData)
{
    ...
    if (spPrevBaton != nullptr)   // first baton is not a slam but the first turn passing 
    {
        ...
        {    
            ...
            CCerrorMT::set("can't create baton object (check if all players deposited containers and slammers or added randoms)");
            return NullUniValue;
        }
    }
    else
    {
        CCerrorMT::set("could not find baton for your pubkey (not your turn)");
        return NullUniValue;
    }
}
...
UniValue KogsAdvertisePlayer(const CPubKey &remotepk, const KogsAdvertising &newad)
{
    ...
    if (spplayer == nullptr || spplayer->objectType != KOGSID_PLAYER)
    {
        CCerrorMT::set("can't load player object");
        return NullUniValue;
    }
    ...
    if (pplayer->encOrigPk != mypk) {
        CCerrorMT::set("not this pubkey player");
        return NullUniValue;
    }
    ...
    if (FindAdvertisings(newad.playerId, adtxid, advout, dummy)) {
        CCerrorMT::set("this player already made advertising");
        return NullUniValue;
    }
...
UniValue KogsCommitRandoms(const CPubKey &remotepk, uint256 gameid, int32_t startNum, const std::vector<uint32_t> &randoms)
{
    ...
    if (!myGetTransaction(gameid, gametx, hashBlock))   {
        CCerrorMT::set("can't load gameid tx");
        return NullUniValue;
    }
    ...
    if (spGameConfig == nullptr || spGameConfig->objectType != KOGSID_GAMECONFIG) {
        CCerrorMT::set("can't load gameconfig tx");
        return NullUniValue;
    }
...
std::cerr << __func__ << " randoms.size()=" << randoms.size() << " pGame->playerids.size()=" << pGame->playerids.size() << " pGameConfig->maxTurns=" << pGameConfig->maxTurns << " total=" << pGame->playerids.size() * pGameConfig->maxTurns * 2 + 1 << std::endl;
    if (randoms.size() < pGame->playerids.size() * pGameConfig->maxTurns * 2 + 1) {
        CCerrorMT::set("insufficient randoms");
        return NullUniValue;
    }
...
//std::cerr << std::endl;
    ...
    if (mpkscommitted.size() != randoms.size()) {
        CCerrorMT::set("no valid committed random txns found");
        return NullUniValue;
    }
    ...
    for(auto const &m : mpkscommitted)  {
        ...
        if (pks != m.second)    {
            CCerrorMT::set("not all pubkeys committed randoms yet for num=" + std::to_string(m.first));
            return NullUniValue;
        }
    }

    if (AddNormalinputsRemote(mtx, mypk, 2*txfee, 0x10000, NULL, true) > 0)
    {
        // spend commit vouts
    ...
    CCerrorMT::set("could not find normal inputs for 2 txfee");
    return NullUniValue; 
}
...
UniValue KogsGetRandom(const CPubKey &remotepk, uint256 gameid, int32_t num)
{
    ...
    if (hashTxns.size() == 0 || randomTxns.size() == 0) {
        CCerrorMT::set("could not get txns");
        return NullUniValue;
    }
    ...
    if (!get_random_value(hashTxns, randomTxns, pks, gameid, num, r, randomUtxos))  {
        CCerrorMT::set("could not get random value");
        return NullUniValue;
    }
...
std::vector<UniValue> KogsUnsealPackToOwner(const CPubKey &remotepk, uint256 packid, vuint8_t encryptkey, vuint8_t iv)
{
    ...
    if (IsNFTBurned(packid, burntx) && !IsGameObjectDeleted(packid))
    {
        ...
        {
            // load pack:
            std::shared_ptr<KogsBaseObject> sppackbaseobj( LoadGameObject(packid, KOGS_TRY_OPRETURN_THEN_SEARCH_TOKEN_VOUT) );
            if (sppackbaseobj == nullptr || sppackbaseobj->objectType != KOGSID_PACK)
            {
                CCerrorMT::set("can't load pack NFT or not a pack");
                return NullResults;
            }
            ...
            {
                CCerrorMT::set("can't decrypt pack content");
                return NullResults;
            }*/
            ...
            /*for (auto tokenid : pack->tokenids)
            {
                ...
                if (!ResultHasTx(sigData)) {
                    results.push_back(MakeResultError("can't create transfer tx (nft could be already sent!): " + CCerrorMT));
                    CCerrorMT.clear(); // clear read CCerrorMT
                }
                else
                    results.push_back(sigData);
            }*/
            ...
            if (results.size() > 0)
            {
                ...
                if (!ResultHasTx(sigData)) {
                    results.push_back(MakeResultError("can't create pack removal tx: " + CCerrorMT::get()));
                    CCerrorMT::clear(); // clear used CCerrorMT
                }
                else
                    results.push_back(sigData);
            }
    ...
    {
        CCerrorMT::set("can't unseal, pack NFT not burned yet or already removed");
    }
    return NullResults;
}
    ...
    if (AddNormalinputsRemote(mtx, mypk, txfee, 0x10000, NULL, true) > 0)
    {
        if (AddTokenCCInputs(cp, mtx, mypk, tokenid, 1, 1, true) > 0)
        {
            ...
            if (ResultHasTx(sigData))
                ...
                CCerrorMT::set("can't finalize or sign burn tx");
        }
        else
            CCerrorMT::set("can't find token inputs");
    }
    else
        CCerrorMT::set("can't find normals for txfee");
    return NullUniValue;
}
...
UniValue KogsRemoveObject(const CPubKey &remotepk, uint256 txid, int32_t nvout)
{
    ...
    if (AddNormalinputsRemote(mtx, mypk, txfee, 0x10000, NULL, true) > 0)
    {
        ...
        if (ResultHasTx(sigData))
            ...
            CCerrorMT::set("can't finalize or sign removal tx");
    }
    else
        CCerrorMT::set("can't find normals for txfee");
    return NullUniValue;
...
UniValue KogsStopAdvertisePlayer(const CPubKey &remotepk, uint256 playerId)
{
    ...
    if (!FindAdvertisings(playerId, adtxid, advout, adlist)) {
        CCerrorMT::set("can't find advertising tx");
        return NullUniValue;
    }
    ...
    if (AddNormalinputsRemote(mtx, mypk, txfee, 0x10000, NULL, true) > 0)
    {
        ...
        if (ResultHasTx(sigData))
            ...
            CCerrorMT::set("can't finalize or sign stop ad tx");
    }
    else
        CCerrorMT::set("can't find normals for txfee");
    return NullUniValue;
...
{
    std::shared_ptr<KogsBaseObject> spobj( LoadGameObject(gameobjectid, KOGS_TRY_OPRETURN_THEN_SEARCH_TOKEN_VOUT) );
    if (spobj == nullptr)
    {
...
void KogsCreateMinerTransactions(int32_t nHeight, std::vector<CTransaction> &minersTransactions)
{
    ...
    for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it = addressUnspents.begin(); it != addressUnspents.end(); it++)
    {
        ...
        if (it->second.satoshis == KOGS_NFT_MARKER_AMOUNT /*&& !myIsutxo_spentinmempool(dummytxid, dummyvout, it->first.txhash, it->first.index)*/) // picking game or slamparam utxos with markers=20000
        {
            ...
            std::shared_ptr<KogsBaseObject> spGameBase(LoadGameObject(it->first.txhash)); // load and unmarshal game 
...
static bool check_baton(struct CCcontract_info *cp, const KogsBaton *pBaton, const CTransaction &tx, std::string &errorStr)
{
    ...
    {                            
        ...
        for (auto const &vout : tx.vout)	{
            ...
            {
                std::shared_ptr<KogsBaseObject> spTokenObj( LoadGameObject(tokenid, KOGS_TRY_OPRETURN_THEN_SEARCH_TOKEN_VOUT) ); // 
        ...
        {
            std::shared_ptr<KogsBaseObject> spToken( LoadGameObject(tokenid, KOGS_TRY_OPRETURN_THEN_SEARCH_TOKEN_VOUT) );
...
static bool check_ops_on_container_addr(struct CCcontract_info *cp, const KogsContainerOps *pContOps, const CTransaction &tx, std::string &errorStr)
{
	...
	std::shared_ptr<KogsBaseObject> spObj( LoadGameObject(pContOps->containerid, KOGS_TRY_OPRETURN_THEN_SEARCH_TOKEN_VOUT) );
	...
	for (auto const &vout : tx.vout)	
    {
        if (vout.scriptPubKey.IsPayToCryptoCondition())
        {
            ...
            if (MyGetCCDropV2(vout.scriptPubKey, drop) && DecodeTokenOpRetV1(drop, tokenid, pks, blobs) != 0)	
            {
                ...
                std::shared_ptr<KogsBaseObject> spMatchObj( LoadGameObject(tokenid, KOGS_TRY_OPRETURN_THEN_SEARCH_TOKEN_VOUT) );  // TODO: maybe check this only on sending?
