...
CScript MarmaraEncodeLoopOpret(uint8_t funcid, uint256 createtxid, CPubKey targetpk, int64_t amount, int32_t matures, std::string currency)
{
    CScript opret; uint8_t evalcode = EVAL_MARMARA;
    opret << OP_RETURN << E_MARSHAL(ss << evalcode << funcid << createtxid << targetpk << amount << matures << currency);
    return(opret);
}

uint8_t MarmaraDecodeLoopOpret(const CScript scriptPubKey, uint256 &createtxid, CPubKey &targetpk, int64_t &amount, int32_t &matures, std::string &currency)
{
    vscript_t vopret; uint8_t *script, e, f;
    GetOpReturnData(scriptPubKey, vopret);
    script = (uint8_t *)vopret.data();
    if (vopret.size() > 2 && E_UNMARSHAL(vopret, ss >> e; ss >> f; ss >> createtxid; ss >> targetpk; ss >> amount; ss >> matures; ss >> currency) != 0)
    {
        return(f);
    }
    return(0);
...
int32_t MarmaraSignature(uint8_t *utxosig, CMutableTransaction &mtx)
{
    ...
    if (GetTransaction(mtx.vin[0].prevout.hash, vintx, hashBlock, false) != 0 && vintx.vout.size() > 1 && mtx.vin[0].prevout.n < vintx.vout.size())
    {
        /*
...
UniValue MarmaraInfo(CPubKey refpk, int32_t firstheight, int32_t lastheight, int64_t minamount, int64_t maxamount, std::string currency)
{
    ...
    result.push_back(Pair("currency", currency));
    if ((n = MarmaraEnumCreditloops(totalamount, issuances, totalclosed, closed, cp, firstheight, lastheight, minamount, maxamount, refpk, currency, [](uint256, int32_t) {/*do nothing*/})) > 0)
    {
        result.push_back(Pair("n", n));
