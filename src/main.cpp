...
static bool ActivateBestChainStep(bool fSkipdpow, CValidationState &state, CBlockIndex *pindexMostWork, CBlock *pblock) {
    ...
    if ( isDpowActive && !fSkipdpow && pindexFork != 0 && pindexOldTip->GetHeight() > notarizedht && pindexFork->GetHeight() < notarizedht )
    {
    ...
    if ( KOMODO_REWIND != 0 )
    {
        ...
        fprintf(stderr,"reached rewind.%d, best to do: ./marmara-cli -ac_name=%s stop\n",KOMODO_REWIND,ASSETCHAINS_SYMBOL);
        sleep(20);
