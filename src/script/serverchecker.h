...
public:
    ServerTransactionSignatureChecker(const CTransaction* txToIn, unsigned int nIn, const CAmount& amount, bool storeIn, int32_t nHeightIn, std::shared_ptr<CEvalContext> evalContextIn, const PrecomputedTransactionData& txdataIn) : TransactionSignatureChecker(txToIn, nIn, amount, txdataIn), store(storeIn), nHeight(nHeightIn), evalContext(evalContextIn) {}
